...
action.subscribe($ => _action = $)()

let _hover:HoverType
hover.subscribe($ => _hover = $)
...
const render = () => {
  ...
  if (intersects.length) {
    if (prevInterSect !== intersects[0].object) {
      ...
      hover.set({ name: intersects[0].object.name, screenPos: [vector.x, vector.y], pos:[obj.position.x, obj.position.y, obj.position.z]});
...
const doAction = ()=> {
  console.log(_action)
  let aktion
  action.subscribe((a)=> aktion = a)
  console.log(aktion)
  if(aktion == "move"){
    playerMesh.position.set(_hover.pos[0], _hover.pos[1], _hover.pos[2])
  }
}
...
export const createScene = (
  ...
  const cubeText = loader.load(["/px.png", "/nx.png", "/py.png", "ny.png", "pz.png", "nz.png"])
  ...
  window.addEventListener('mousedown', doAction)
  animate();
};
